...
export const LemonadeDiscoveryPlugin = async (
  ...
  return {
    config: async (config: {
      ...
      const providerDiscoveryOpts = (providerOpts.discovery ??
        {}) as LemonadeDiscoveryOptions;

      const mergedOpts: LemonadeDiscoveryOptions = {
        downloaded_only: true,
        exclude_labels: ["embedding", "tts", "stt"],
        ...providerDiscoveryOpts,
        ...opts,
      };

      if (providerDiscoveryOpts.exclude_labels) {
        mergedOpts.exclude_labels = Array.from(
          new Set([
            "embedding",
            "tts",
            "stt",
            ...providerDiscoveryOpts.exclude_labels,
          ]),
        );
      }
      ...
      if (mergedOpts.small_model) {
        const smallModel = resolveValue(mergedOpts.small_model);
        if (smallModel) {
      ...
      const prov = config.provider[providerId];
      if (!prov.models) {
        prov.models = {};
      }
